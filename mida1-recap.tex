...
\begin{itemize}
	\item \bm{$AR(1)$}: $\hat{v}(t+r|t)=a^rv(t)$  (or $\hat{v}(t|t-r)=a^rv(t-r)$)
	\item \bm{$MA(1)$}: $\hat{v}(t+1|t)=-c\hat{v}(t|t-1)+v(t), \forall k > n$ \\ or \space \space \space \space \space \space \space \space \space $\hat{v}(t|t-1)=-c\hat{v}(t-1|t-2)+v(t-1), \forall k > n$
...
\begin{enumerate}
	...
	\item \question{Consider the stochastic process as \boldmath{$y(t)=e(t)+0.5e(t-1)-0.5e(t-2)$} where $e(t)$ is a white noise with expected value 1 and variance 1.}{The expected value of the process is 0.}{The expected value of the process is 1.}{The variance of the process is 3/2.}{The variance of the process is 5/4.}
	\item \question{For a given time series, it turns out that the estimated Partial Covariance function $PARCOV(\tau)$ becomes null for $\tau\geq5$. Which is the appropriate model to fit data?}{$MA(5)$}{$MA(4)$}{$AR(4)$}{$ARMA(3,3)$}
\end{enumerate}
\pagebreak
